...
6. **Что взять?** «Взять с собой» у каждого места и в планировщике: способы под сезон и лёд с конкретными приманками и оснастками из словаря; отмеченное в «моём ящике» получает галочку. Плюс чек-листы по способу и сезону, безопасность на льду, клещи, гроза.
...
| Check | Result |
|-------|--------|
| `npm test` | 41 unit tests pass (model, point estimate, gear advice, marking decoder) |
| `npm run qa` | ok — 28 species, 56 spots, 29 zones, 40 gear cards, payload 2.77 MB |
