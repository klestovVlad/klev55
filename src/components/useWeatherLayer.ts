...
export function buildWeatherGeo(grid: WeatherGrid | undefined, date: Date) {
  ...
  const samples = (prop: 'precip' | 'cloud') => pts.map((p) => ({ lon: p.geometry.coordinates[0], lat: p.geometry.coordinates[1], value: p.properties[prop] as number }));
  return { points, rain: idwBands(samples('precip'), RAIN_BREAKS, { property: 'precip' }), cloud: idwBands(samples('cloud'), CLOUD_BREAKS, { property: 'cloud' }) };
}
